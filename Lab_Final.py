...
class Page2(Page):
    ...
    def __init__(self, *args, **kwargs):
       global canvas,label1, entry_ajust, lim1,lim2, ax1,fig, cid
       Page.__init__(self, *args, **kwargs)
    ...
    ###################################################### BOTONES #########################################
       ...
       btn_barrido_cont=tk.Button(self, text='Añadir ajuste', command= Page2.Añadir)
       btn_barrido_cont.grid(row=10,column=0,columnspan=5,ipadx=5,ipady=15)

       btn_barrido_cont=tk.Button(self, text='Suprimir ajuste', command= Page2.Suprimir)
       btn_barrido_cont.grid(row=10,column=1,columnspan=5,ipadx=5,ipady=15)
    ...
    def Ajustar():
        global coords, contador,lim1,lim2, ax1, lista_results,Ajuste_actual
...
class Page3(Page):
    ...
    # Habré de descomentar esta parte para el programa de lab real
        global contador_int,Datos_3D,OnOff
        orden=[] #Lista con las órdenes que se le van a pasar al módulo láser
        OnOff=1  
